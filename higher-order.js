...
let evenNumbers = mixedNumbers.filter((element, index, arr) => element % 2 == 0)
...
let postTaxPrices = prices.map((element,index,arr) => element * 1.07)
...
let totalPopulation = populations.reduce((acc, element, index, arr) => acc + element)
...
let myStrongest = monstersInYourPocket.filter((element,index,arr) => element.CP > 200)
...
let ordersTotal = orders.map((element, index, arr) => (element.tax + 1.00) * element.price)
...
let bobsTotal = purchases.reduce((acc, next, index, array) => {
   return next.owner === "Bob" ? acc + next.price : acc
},0)
